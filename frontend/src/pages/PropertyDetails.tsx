    ...
    <section className="px-4 py-6 pb-28 max-w-4xl mx-auto bg-[#F5F1E3] min-h-[calc(100vh-64px)] text-[#050505]">
      ...
      <div className="bg-white rounded-2xl shadow-md p-4 mb-6">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xl font-bold text-[#008080]">
            {property.priceMonthly ?? 'Precio no disponible'}
          </span>
          <span className="text-xs bg-[#DDDBCB] text-[#050505] rounded-full px-2 py-1">
            ⭐ {property.rating?.toFixed(1) ?? '4.7'}
          </span>
        </div>
        <h2 className="text-xl font-semibold mb-1">{property.title ?? 'Sin título'}</h2>
        <p className="text-sm text-[#050505]/70 mb-3">
          {property.subtitle ?? 'Ubicación aproximada'}
        </p>
      ...
      <div className="bg-white rounded-2xl shadow-md p-4 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="badge bg-[#DDDBCB] text-[#050505] border-0">
            {occupants.length} inquilinos
          </span>
        </div>
        {occupants.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-3">
            {occupants.map((person) => (
              <div
                key={person.name}
                className="flex items-center gap-3 bg-[#DDDBCB] rounded-full px-3 py-2"
              >
                ...
                <div className="text-sm">
                  ...
                  <p className="text-xs text-[#050505]/70">
                    {person.age ? `${person.age} años` : 'Edad privada'}
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="mt-1">
            ...
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M7 11V8a5 5 0 0110 0v3m-9 0h8a2 2 0 012 2v5a2 2 0 01-2 2H8a2 2 0 01-2-2v-5a2 2 0 012-2z"
              />
            </svg>
          </div>
          <div>
            ...
            <p className="text-xs text-[#050505]/70">
              Datos sensibles bloqueados hasta Match verificado
            </p>
          </div>
        </div>
        <div className="space-y-3">
          <div
            className={`flex items-center justify-between rounded-xl bg-white px-4 py-3 ${isPrivacyUnlocked ? '' : 'opacity-60'}`}
          >
            <span className={`text-sm text-[#050505]/70 ${isPrivacyUnlocked ? '' : 'blur-sm'}`}>
              Dirección exacta
            </span>
        ...
        {amenities.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2">
            {amenities.map((amenity) => (
              <span
                key={amenity}
                className="badge bg-[#DDDBCB] text-[#050505] border-0 text-sm py-3 px-4 rounded-full"
              >
                {amenity}
              </span>
            ))}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#DDDBCB] shadow-[0_-8px_24px_rgba(0,0,0,0.08)]">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-3">
          <div className="flex-1">
            ...
            <p className="text-lg font-semibold text-[#008080]">
              {property.priceMonthly ?? 'Precio no disponible'}
            </p>
          </div>
          <button
            onClick={handleLike}
            className="btn flex-1 h-12 text-base bg-[#008080] hover:bg-[#006b6b] text-white border-0 rounded-2xl shadow-lg w-full"
          >
            Solicitar Like
          </button>
        </div>
