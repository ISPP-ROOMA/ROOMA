...
export default function MyRequests() {
  ...
  return (
    <div data-theme="light" className="mx-auto w-full max-w-md min-h-dvh bg-[#F5F1E3] text-[#050505] pb-28">
      <header className="sticky top-0 z-10 bg-[#F5F1E3] px-4 pt-5 pb-4">
        <div className="flex items-center justify-between">
          <button className="h-10 w-10 rounded-full bg-white text-[#050505] shadow-sm flex items-center justify-center" aria-label="Volver">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M15 18l-6-6 6-6" />
            </svg>
          </button>
          <h1 className="text-3xl font-bold text-[#050505]">Mis Solicitudes</h1>
          <button className="h-10 w-10 rounded-full bg-white text-[#050505] shadow-sm flex items-center justify-center" aria-label="Notificaciones">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <section className="mt-5 space-y-4 px-4">
        ...
        {visibleItems.map((item) => (
          ...
          >
            <div className={`relative h-44 w-full ${item.status === "cancelled" ? "grayscale" : ""}`}>
              <img src={item.imageUrl} alt={item.title} className="h-full w-full object-cover" />
              {item.status === "cancelled" && (
                <span className="absolute right-3 top-3 rounded-lg bg-white/90 px-2 py-1 text-sm font-semibold text-[#050505]/60">Cancelada</span>
              )}
            </div>
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`text-xl font-semibold ${item.status === "cancelled" ? "text-[#050505]/60" : "text-[#008080]"}`}>
                  {item.price}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between gap-2 text-sm">
                <span className={`rounded-full px-3 py-1 text-sm ${statusClass(item.status)}`}>{statusLabel(item.status)}</span>
