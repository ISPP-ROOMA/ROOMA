      ...
      <main className={MAIN_CLASS}>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">Vuelve más tarde para ver nuevas publicaciones.</p>
          </div>
        ) : (
            ...
            {apartments.map((apt) => (
              ...
              >
                ...
                <div className="p-5 flex flex-col gap-2 flex-1">
                  <h3 className="font-bold text-base text-base-content line-clamp-1">{apt.title}</h3>
                  <p className="text-sm text-gray-500 line-clamp-2">{apt.description}</p>
                  <div className="flex items-center justify-between mt-auto pt-2">
                    <span className="text-xl font-extrabold text-primary">{apt.price} €<span className="text-sm font-normal text-gray-400"> / mes</span></span>
                    <span className="text-xs text-gray-400">{apt.ubication}</span>
                  </div>
                  {apt.bills && (
                    <p className="text-xs text-gray-400">Gastos: {apt.bills}</p>
                  )}
                </div>
              </Link>
            ))}
